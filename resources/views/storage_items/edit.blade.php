                ...
                <div class="card border-0 shadow-sm mb-6">
                    ...
                    <div class="card-body p-6">
                        ...
                        <div class="row g-5 mb-8">
                            <div class="col-md-6">
                                <label class="form-label required fw-semibold fs-6">Almacén</label>
                                <div class="d-flex gap-2">
                                    <select name="storage_warehouse_id" id="storageWarehouseSelect"
                                        class="form-select form-select-solid @error('storage_warehouse_id') is-invalid @enderror" required>
                                        @foreach ($warehouses as $warehouse)
                                            <option value="{{ $warehouse->id }}" {{ (int) old('storage_warehouse_id', $item->storage_warehouse_id ?: $defaultWarehouseId) === (int) $warehouse->id ? 'selected' : '' }}>
                                                {{ $warehouse->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <button class="btn btn-light-primary" type="button" data-bs-toggle="modal" data-bs-target="#createWarehouseModal">+</button>
                                </div>
                            ...
                            <div class="col-md-6">
                                <label class="form-label required fw-semibold fs-6">Zona</label>
                                <div class="d-flex gap-2">
                                    <select name="storage_zone_id" id="storageZoneSelect"
                                        class="form-select form-select-solid @error('storage_zone_id') is-invalid @enderror" required></select>
                                    <button class="btn btn-light-primary" type="button" data-bs-toggle="modal" data-bs-target="#createZoneModal">+</button>
                                </div>
...
@push('scripts')
    <script>
        (() => {
            ...
            const initialZoneId = @json((int) old('storage_zone_id', $item->storage_zone_id ?: $defaultZoneId));
            ...
            const renderZones = (warehouseId, preferred = null) => {
                ...
                zones.forEach((z) => {
                    ...
                    if (String(preferred ?? initialZoneId) === String(z.id)) option.selected = true;
